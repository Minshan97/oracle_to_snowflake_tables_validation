...
def compare_values(v1, v2):
    ...
    if normalize_value(v1) == normalize_value(v2): return 'Warning'
    try:
        set1, set2 = set(str(v1).split(',')), set(str(v2).split(','))
        if normalize_value(','.join(set1)) == normalize_value(','.join(set2)): return 'Warning'
    except: pass
    return 'Fail'
